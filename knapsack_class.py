...
def print_evaluation_values(solution:tuple[Knapsack,list[Item]], capacity:int):
    """Evaluering av løsning\n"""
    print(f"\tVerdi per vekt: {best_eval(solution[0], capacity)[0]/sum([item.weight for item in solution[0].items])}")
    print(f"\tFitness vekt: {best_eval(solution[0], capacity)[1]}")
